...
def fetch_api_data():
    # 1. 전력수급현황 (더미 데이터를 실제 API 수신 구조로 설계)
    # 실제 연동 시: res = requests.get(url).json()
...
supply_val = pwr_data['supply'] # API 실시간 공급 능력
now_hour = now.hour
actual_load = [l + np.random.uniform(-1.0, 1.0) if i <= now_hour else None for i, l in enumerate(load_forecast)]
reserve_gw_list = [supply_val - (a if a is not None else f) for a, f in zip(actual_load, load_forecast)] # 운영 예비력 계산
...
# 하단 안내 문구
if status == "정상":
    st.success(f"✅ 현재 전력 예비력이 {reserve_gw}GW로 매우 넉넉합니다. 수급 상황이 안정적입니다.")
else:
    st.warning(f"⚠️ 현재 수급 상태가 '{status}' 단계입니다. 예비율 추이를 면밀히 모니터링하세요.")
